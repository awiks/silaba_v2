...
class ContactController extends Controller
{
    ...
    public function store(ContactRequest $contactRequest)
    {
        try {
            ...
            $validate['contact_type'] = json_encode($contactRequest->contact_type);
    ...
    public function update(ContactRequest $contactRequest, Contact $contact)
    {
        try {
            ...
            if ($contactRequest->hasFile('profile')) {
                ...
                $contact->profile != null ? unlink(storage_path($contact->profile)) : '';
        ...
        try {
            if( $request->id == null ){
                ...
                if( $request->input('restore') === 'restore' ){
                    ...
                    return redirect('contact')->with('success', 'Data berhasil dipulihkan');
                }
                elseif( $request->input('forever') === 'forever' ){
                    ...
                    Contact::whereIn('id',$request->id)->forcedelete();
                     ...
                     return redirect('contact')->with('success', 'Data berhasil dihapus secara permanen');
                 }
             }
         } catch (\Throwable $th) {
             return redirect('contact')->with('error', 'Data gagal diperbarui');
