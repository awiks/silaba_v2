...
<div class="card elevation-0">
    <div class="card-header">
        ...
        <button class="btn btn-primary" onclick="location.href='{{ url('setting/brand/create') }}'">
            <i class="fas fa-plus-circle"></i> Tambah
        </button>
        <div class="card-tools">
            <button type="button" class="btn btn-default" onclick="location.href='{{ url('/setting/brand/recycle_bin') }}'">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-info">
            Disarankan untuk tidak mengubah atau menghapus daftar merek jika sudah terjadi transaksi.
        </div>
                ...
                <tbody>
                    @foreach ($brand as $value)
                        <tr>
                            ...
                            <td>
                                <a href="{{ url("/setting/brand/{$value->id}/edit") }}" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
...
<script type="text/javascript">
    ...
    swal({
        ...
        if (isConfirm) {
            $.ajax({
                url: '/setting/brand/'+id+'',
                type: 'delete',
                dataType: 'json',
                data: { id: id,_token: '{{ csrf_token() }}',},
                success:function(data){
                    if( data.status == 1 ){
                        window.location.href = "{{ url('setting/brand') }}";
                    }
                    else{
                        location.reload();
                    }
