...
<div class="card elevation-0">
    <div class="card-header">
        ...
        <button class="btn btn-primary" onclick="location.href='{{ url('setting/account_sub_header/create') }}'">
            <i class="fas fa-plus-circle"></i> Tambah
        </button>
        <div class="card-tools">
            <button type="button" class="btn btn-default" onclick="location.href='{{ url('/setting/account_sub_header/recycle_bin') }}'">
            ...
            <table class="table table-striped" id="DataTable" width="100%">
                ...
                <tbody>
                    @foreach ($account_sub_header as $value)
                        <tr>
                            <td>{{ $account_sub_header->firstItem() + $loop->index }}</td>
                            ...
                            <td>
                                <a href="{{ url('/setting/account_sub_header/'.$value->id.'/edit') }}" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
...
<script type="text/javascript">
    ...
    swal({
        ...
        if (isConfirm) {
            $.ajax({
                url: '/setting/account_sub_header/'+id+'',
                type: 'delete',
                dataType: 'json',
                data: { id: id,_token: '{{ csrf_token() }}',},
                success:function(data){
                    if( data.status == 1 ){
                        window.location.href = "{{ url('setting/account_sub_header') }}";
                    }
                    else{
                        location.reload();
                    }
