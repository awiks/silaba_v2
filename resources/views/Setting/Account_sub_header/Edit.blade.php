...
@section('title',$title)
@section('content')
<form action="{{ url('setting/account_sub_header/'.$account_sub_header->id.'') }}" method="post">
    @method('PUT')
    @csrf
<div class="card elevation-0">
    <div class="card-body">
        @php $header_id_old = old('header_id') ? old('header_id') : $account_sub_header->header_id; @endphp
        <div class="form-group">
            <label>Header</label>
            <select name="header_id" class="form-control @error('header_id') is-invalid @enderror">
                <option value="">-- Pilih --</option>
                @foreach ($account_header as $value)
                <option value="{{ $value->id }}" @if ( $value->id == $header_id_old   ) selected @endif>{{ $value->header_code.' - '.$value->header_name }}</option>
                @endforeach
            </select>
            @error('header_id')
        ...
        <div class="form-group">
            ...
            <input type="text" name="sub_header_code" value="{{ old('sub_header_code') ? old('sub_header_code') : $account_sub_header->sub_header_code }}" class="form-control @error('sub_header_code') is-invalid @enderror">
        ...
        <div class="form-group">
            ...
            <input type="text" name="header_sub_name" value="{{old('header_sub_name') ? old('header_sub_name') : $account_sub_header->header_sub_name }}" class="form-control @error('header_sub_name') is-invalid @enderror">
