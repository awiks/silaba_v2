...
<div class="card elevation-0">
    <div class="card-header">
        ...
        <button class="btn btn-primary" onclick="location.href='{{ url('setting/unit/create') }}'">
            <i class="fas fa-plus-circle"></i> Tambah
        </button>
        <div class="card-tools">
            <button type="button" class="btn btn-default" onclick="location.href='{{ url('/setting/unit/recycle_bin') }}'">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-info">
            Disarankan untuk tidak mengubah atau menghapus daftar satuan jika sudah terjadi transaksi.
        </div>
            ...
            <table class="table table-striped" id="DataTable" width="100%">
                ...
                <tbody>
                    @foreach ($unit as $value)
                        <tr>
                            <td>{{ $unit->firstItem() + $loop->index }}</td>
                            <td>{{ $value->unit_name }}</td>
                            <td>{{ date('d-m-Y H:i',strtotime($value->updated_at)) }}</td>
                            <td>
                                <a href="{{ url("/setting/unit/{$value->id}/edit") }}" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
        ...
        if (isConfirm) {
            $.ajax({
                ...
                success:function(data){
                    if( data.status == 1 ){
                        window.location.href = "{{ url('setting/unit') }}";
                    }
                    else{
                        location.reload();
                    }
